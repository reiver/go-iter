...
func (receiver *Uint8) Close() error {
	if nil == receiver {
...
func (receiver *Uint8) Decode(x interface{}) error {
	if nil == receiver {
		return errNilReceiver
	}
	...
	case *uint16:
		...
		*p = uint16(receiver.datum)
	case *uint32:
		if nil == p {
			return nil
		}

		*p = uint32(receiver.datum)
	case *uint64:
		if nil == p {
			return nil
		}
