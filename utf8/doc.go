/*
Package iterutf8 provides tools for creating iterators for the []byte type, where the data in the []byte is encoded as UTF-8;
note that values decoded from this are of the rune type (and not the byte type).
...
For example, we can turn a slice into an iterator, with code like the following:
	...
	for iterator.Next() {
		...
		var datum rune // Could have also used: var datum interface{}
